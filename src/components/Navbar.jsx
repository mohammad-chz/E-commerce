        ...
        <header className='relative flex items-center justify-between bg-orange-100 py-4 px-14   max-sm:gap-4 max-sm:py-2 max-sm:px-4' id='navbar' ref={navbarRef}>
            ...
            <div className='hidden md:flex items-center justify-center gap-6'>
                ...
                <div className='relative'>
                    <Link onClick={() => setMenu("cart")} to='/cart'><img className={`${menu === 'cart' ? 'w-14' : 'w-10'}`} src={images.iconCart} alt="iconCart Logo" /></Link>
            ...
            <div className='flex md:hidden items-center justify-center gap-4'>
                <Link onClick={() => setMenu("home")} to='/'><img className='block md:hidden w-14' src={images.logo} alt="Cart Logo" /></Link>
                <img className='w-7' src={images.search} alt="Search Logo" />
                <Link onClick={() => setMenu("cart")} to='/cart'><img className='w-7' src={images.iconCart} alt="iconCart Logo" /></Link>
            ...
            <ul
                ...
                <Link to='/' onClick={() => handleCloseMenu('home')}><li className={`${menu === "home" ? "text-red-500 font-bold" : ""} hover:text-orange-400 cursor-pointer p-2`}>خانه</li></Link>
                <li onClick={() => handleCloseMenu('menu')} className={`${menu === "menu" ? "text-red-500 font-bold" : ""} hover:text-orange-400 cursor-pointer p-2`}>منو</li>
                <li onClick={() => handleCloseMenu('app')} className={`${menu === "app" ? "text-red-500 font-bold" : ""} hover:text-orange-400 cursor-pointer p-2`}>برنامه موبایل</li>
                <li onClick={() => handleCloseMenu('contact')} className={`${menu === "contact" ? "text-red-500 font-bold" : ""} hover:text-orange-400 cursor-pointer p-2`}>تماس با ما</li>
            </ul>
        </header>
    )
}
